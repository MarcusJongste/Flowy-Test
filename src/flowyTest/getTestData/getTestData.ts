...
function getTestData():Promise<testDoc> {
    const context = require.context('../../flowyCore', true, /\.(ts|test.ts)$/, 'lazy');
    const testDocument:testDoc = {};
    return Promise.all(context.keys().map((filePath: string, i: number) => {
        if (filePath[0] !== '.' && filePath.indexOf('index.ts') === -1) {
            const splitPath = filePath.split('/'),
                fileName: string = splitPath[splitPath.length - 1],
                testFileName: string = fileName.substring(0, fileName.indexOf('.')),
                module: string = splitPath[splitPath.length - 2],
                namespace: string = splitPath[splitPath.length - 3] || 'root';
